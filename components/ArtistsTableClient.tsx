"use client";

import React from "react";
import ArtistModal from "./ArtistModal";

type Row = {
  id: string;
  name: string;
  totalStreams: number;
  thisWeek: number;
  prevWeek: number;
};

type SortKey = "name" | "total" | "growth" | "thisWeek";
type SortDir = "asc" | "desc";

export default function ArtistsTableClient({ rows }: { rows: Row[] }) {
  const [query, setQuery] = React.useState("");
  const [sortKey, setSortKey] = React.useState<SortKey>("growth");
  const [sortDir, setSortDir] = React.useState<SortDir>("desc");
  const [selected, setSelected] = React.useState<{ id: string; name: string } | null>(null);

  // pre-starred from backend
  const [starred, setStarred] = React.useState<Set<string>>(new Set());
  React.useEffect(() => {
    fetch("/api/watchlist")
      .then((r) => r.json())
      .then((data) => {
        const ids: string[] = (data?.items ?? data ?? []).map((x: any) => x.id || x.artistId);
        setStarred(new Set(ids));
      })
      .catch(() => {});
  }, []);

  async function toggleStar(id: string) {
    const next = !starred.has(id);
    setStarred((prev) => {
      const s = new Set(prev);
      if (next) s.add(id);
      else s.delete(id);
      return s;
    });
    try {
      await fetch("/api/star", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ artistId: id, star: next }),
      });
    } catch {}
  }

  // enrich with growth
  const enriched = React.useMemo(
    () =>
      rows.map((r) => ({
        ...r,
        growthRatePct: r.prevWeek > 0 ? ((r.thisWeek - r.prevWeek) / r.prevWeek) * 100 : 0,
      })),
    [rows]
  );

  // search
  const filtered = React.useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return enriched;
    return enriched.filter((r) => r.name.toLowerCase().includes(q));
  }, [enriched, query]);

  // sort
  function applySort(nextKey: SortKey) {
    if (nextKey === sortKey) {
      setSortDir((d) => (d === "asc" ? "desc" : "asc"));
    } else {
      setSortKey(nextKey);
      setSortDir(nextKey === "name" ? "asc" : "desc");
    }
  }

  const sorted = React.useMemo(() => {
    const copy = [...filtered];
    copy.sort((a, b) => {
      let va: number | string = 0;
      let vb: number | string = 0;

      switch (sortKey) {
        case "name":
          va = a.name.toLowerCase();
          vb = b.name.toLowerCase();
          break;
        case "total":
          va = a.totalStreams ?? 0;
          vb = b.totalStreams ?? 0;
          break;
        case "growth":
          va = a.growthRatePct ?? 0;
          vb = b.growthRatePct ?? 0;
          break;
        case "thisWeek":
          va = a.thisWeek ?? 0;
          vb = b.thisWeek ?? 0;
          break;
      }

      if (typeof va === "string" && typeof vb === "string") {
        const cmp = va.localeCompare(vb);
        return sortDir === "asc" ? cmp : -cmp;
      } else {
        const cmp = (vb as number) - (va as number);
        return sortDir === "asc" ? -cmp : cmp;
      }
    });
    return copy;
  }, [filtered, sortKey, sortDir]);

  const sortArrow = (key: SortKey) =>
    key !== sortKey ? "" : sortDir === "asc" ? " ▲" : " ▼";

  return (
    <div className="space-y-4">
      {/* Search */}
      <input
        className="w-full md:max-w-2xl px-3 py-2 rounded border border-zinc-700 bg-zinc-900 text-zinc-100 placeholder-zinc-500"
        placeholder="Search artists..."
        value={query}
        onChange={(e) => setQuery(e.target.value)}
      />

      {/* Table */}
      <div className="rounded border border-zinc-800 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="text-zinc-400 bg-zinc-900/60">
            <tr className="border-b border-zinc-800">
              <th className="text-left p-3">
                <button
                  onClick={() => applySort("name")}
                  className="font-medium hover:text-white"
                >
                  Artist{sortArrow("name")}
                </button>
              </th>
              <th className="text-left p-3">
                <button
                  onClick={() => applySort("total")}
                  className="font-medium hover:text-white"
                >
                  Total Streams{sortArrow("total")}
                </button>
              </th>
              <th className="text-left p-3">
                <button
                  onClick={() => applySort("growth")}
                  className="font-medium hover:text-white"
                >
                  Growth Rate{sortArrow("growth")}
                </button>
              </th>
              <th className="text-left p-3">
                <button
                  onClick={() => applySort("thisWeek")}
                  className="font-medium hover:text-white"
                >
                  This Week{sortArrow("thisWeek")}
                </button>
              </th>
              <th className="p-3 text-right">Watch</th>
            </tr>
          </thead>
          <tbody>
            {sorted.map((r) => {
              const growth = r.growthRatePct ?? 0;
              const growthColor =
                growth > 0 ? "text-emerald-400" : growth < 0 ? "text-red-400" : "text-zinc-400";
              const isStarred = starred.has(r.id);

              return (
                <tr
                  key={r.id}
                  className="border-b border-zinc-800 hover:bg-zinc-900/50 transition-colors"
                >
                  <td
                    className="p-3 cursor-pointer hover:underline"
                    onClick={() => setSelected({ id: r.id, name: r.name })}
                  >
                    {r.name}
                  </td>
                  <td className="p-3">{fmt(r.totalStreams)}</td>
                  <td className={`p-3 ${growthColor}`}>
                    {growth === 0 ? "—" : `${growth > 0 ? "▲" : "▼"} ${Math.abs(growth).toFixed(2)}%`}
                  </td>
                  <td className="p-3">{fmt(r.thisWeek)}</td>
                  <td className="p-3 text-right">
                    <button
                      onClick={() => toggleStar(r.id)}
                      className={`inline-flex items-center justify-center w-7 h-7 rounded border ${
                        isStarred
                          ? "border-yellow-500 bg-yellow-500/10 text-yellow-400"
                          : "border-zinc-700 text-zinc-300 hover:border-zinc-500"
                      }`}
                      title={isStarred ? "Remove from watchlist" : "Add to watchlist"}
                    >
                      ★
                    </button>
                  </td>
                </tr>
              );
            })}
            {sorted.length === 0 && (
              <tr>
                <td colSpan={5} className="p-6 text-zinc-400">
                  No artists match your filters.
                </td>
              </tr>
            )}
          </tbody>
        </table>
      </div>

      {selected && <ArtistModal artist={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}

function fmt(n?: number) {
  if (!n || n === 0) return "—";
  if (n >= 1_000_000) return `${(n / 1_000_000).toFixed(1)}M`;
  if (n >= 1_000) return `${(n / 1_000).toFixed(1)}K`;
  return n.toLocaleString();
}
